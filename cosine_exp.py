...
def cosine_similarity_function(a, b):
    ...
    # return np.mean(a.cpu().numpy() @ b.cpu().numpy().T)
...
def run_experiment(model_name, save_path, train_dataset, dataset_name, features, finetune, config_path, bias_metric, pca_comps, trend_analysis=None, only_pretrained=False, multiple_trials=False, cross_analysis=False):
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)
    labels = config['LABELS_NAMES']
    comps = config['COMPS']
